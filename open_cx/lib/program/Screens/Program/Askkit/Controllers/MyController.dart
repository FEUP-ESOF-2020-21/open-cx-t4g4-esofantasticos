...
class MyController implements DatabaseController {
  ...
  Future<Answer> addAnswer(Question question, String content, bool anonymous) {
    answers.add(
        new Answer(currentUser, content, DateTime.now(), anonymous, question));
  ...
  Future<Question> addQuestion(Talk talk, String content, bool anonymous) {
    questions.add(
        new Question(talk, currentUser, content, DateTime.now(), anonymous));
  ...
  Future<List<Answer>> getAnswers(Question question) {
    List<Answer> questionAnswers =
        answers.where((answer) => question == answer.question).toList();
    questionAnswers.sort((answer1, answer2) {
      if (answer2.bestAnswer == answer1.bestAnswer) return 0;
  ...
  Future<List<Question>> getQuestions(Talk talk) async {
    List<Question> talkQuestions =
        questions.where((question) => question.talk == talk).toList();
  ...
  Future<int> getUserUpvote(Question question) {
    List<Upvote> userUpvotes = upvotes
        .where((upvote) =>
            upvote.question == question && upvote.user == currentUser)
        .toList();
    if (userUpvotes.length == 0) return Future.value(0);
  ...
  Future<void> setUserUpvote(Question question, int value) async {
    ...
    if (userUpvotes.length == 0)
      ...
      userUpvotes[0].value = value;
    question.upvotes = await this.getUpvotes(question);
  }

  @override
  Future<int> getUpvotes(Question question) {
    List<Upvote> questionUpvotes =
        upvotes.where((upvote) => upvote.question == question).toList();
    int sum = 0;
  ...
  @override
  Future<void> addRating(Talk talk, double rating) {
