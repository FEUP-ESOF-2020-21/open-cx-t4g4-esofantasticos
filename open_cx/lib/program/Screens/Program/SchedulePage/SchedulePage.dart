...
class SchedulePageState extends State<SchedulePage> {
  ...
  List<DateTime> daysList = [
    new DateTime(2019, 12, 8, 8, 0),
    new DateTime(2019, 12, 9, 8, 0),
    new DateTime(2019, 12, 10, 8, 0),
    new DateTime(2019, 12, 11, 8, 0),
    new DateTime(2019, 12, 12, 8, 0),
    new DateTime(2019, 12, 13, 8, 0)
  ];
  List<ScrollController> controllers = new List<ScrollController>();

  List<String> timeInterval = new List<String>();

  List<String> createIntervals(
      int firstHour, int firstMinutes, int lastHour, int lastMinutes) {
    String firstTime = firstHour.toString().padLeft(2, '0') +
        ":" +
        firstMinutes.toString().padLeft(2, '0');
    String lastTime = lastHour.toString().padLeft(2, '0') +
        ":" +
        lastMinutes.toString().padLeft(2, '0');
    ...
    if (firstMinutes != 0) differenceBlocks -= 1;

    if (lastMinutes != 0) differenceBlocks += 1;
    ...
    for (int i = 1; i < differenceBlocks; i++) {
      ...
      finalList.add(currentHour.toString().padLeft(2, '0') +
          ":" +
          currentMinutes.toString().padRight(2, '0'));
    }
...
// Usado para facilitar a colocaçao dos blocos na tabela
  ...
  Widget individualTalkBlock(int i, int j, Color backColor, Color blockColor) {
    ...
    convertToBlocks(
        widget.talkList[j].dateInitial, widget.talkList[j].dateFinal);
    return Container(
        child: Column(
      children: <Widget>[
        Container(
        ...
        GestureDetector(
          onTap: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => TalkPage(talk: widget.talkList[j])));
          },
          child: Container(
        ...
        children: <Widget>[
          for (int j = 0; j < widget.talkList.length; j++)
            if (daysList[i].day == widget.talkList[j].dateInitial.day)
              individualTalkBlock(
                  i, j, color, Color.fromARGB(255, 247, 220, 222)),
          Container(
      ...
      child: Stack(
        children: <Widget>[
          ListView(
            ...
            children: <Widget>[
              for (int i = 0; i < daysList.length; i++) dayContainer(i),
            ],
          ),
          Container(
            child: ListView(
